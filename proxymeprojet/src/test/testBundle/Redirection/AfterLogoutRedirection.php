...
class AfterLogoutRedirection implements LogoutSuccessHandlerInterface
{
    ...
    public function onLogoutSuccess(Request $request)
    {
        ...
        if (in_array('ROLE_SOCIETE', $rolesTab, true) || in_array('ROLE_ADMIN', $rolesTab, true) || in_array('ROLE_SUPER_ADMIN', $rolesTab, true))
            $response = new RedirectResponse($this->router->generate('hy'));
        // otherwise we redirect user to the homepage of website
        else
            $response = new RedirectResponse($this->router->generate('hy7'));
